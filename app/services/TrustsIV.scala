...
class TrustsIV @Inject() (trustsAuth: TrustsAuthorisedFunctions) extends Logging {
  ...
  def authenticate[A](
    ...
    val relationship = identifier match {
      case UTR(utr) =>
        Relationship(
          trustsAuth.config.relationshipName,
          Set(BusinessKey(trustsAuth.config.taxableRelationshipIdentifier, utr))
        )
      case URN(urn) =>
        Relationship(
          trustsAuth.config.relationshipName,
          Set(BusinessKey(trustsAuth.config.nonTaxableRelationshipIdentifier, urn))
        )
    }
    ...
    trustsAuth.authorised(relationship) {
      ...
      logger.info(
        s"[TrustsIV][authenticate][Session ID: ${Session.id(hc)}][UTR/URN: $identifier]" +
